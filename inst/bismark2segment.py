...
def process_segment(CpG_file, path="./"):
    ...
    for root, dirs, files in os.walk(path):
        for file in files:
            if file.endswith(".bis_.segment_"):
                filename = os.path.join(root, file)
                ...
                if search:
                    ...
                    segment.iloc[index, 0] = (
                        one_chr
                        + ":"
                        + (loci_mat.iloc[index, 0].apply(int) - 1).apply(str)
                        + "_"
                        + (loci_mat.iloc[index, 1].apply(int) - 1).apply(str)
                        + "_"
                        + (loci_mat.iloc[index, 2].apply(int) - 1).apply(str)
                        + "_"
                        + (loci_mat.iloc[index, 3].apply(int) - 1).apply(str)
                    )
                    temp_len = len(cpg_index)
                    temp_cpg.index = range(0, temp_len)
                    loci_1 = temp_cpg.iloc[0: (temp_len - 3), 1]
                    loci_2 = temp_cpg.iloc[1: (temp_len - 2), 1]
                    loci_3 = temp_cpg.iloc[2: (temp_len - 1), 1]
    ...
    for root, dirs, files in os.walk(path):
        for filename in files:
            if filename.endswith(".segment_"):
                ...
                segment.iloc[index, 0] = (
                    segment["chrom_loci"].str.split(":", expand=True)[0]
                    + ":"
                    + (loci_mat.iloc[index, 0].apply(int) - 1).apply(str)
                    + "_"
                    + (loci_mat.iloc[index, 1].apply(int) - 1).apply(str)
